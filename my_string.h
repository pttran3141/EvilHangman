...
//Postcondition: The memory used for the MY_STRING object has to be reclaimed by the system and the handle referred to by the pointer phMy_string has been set to NULL.
...
//Postcondition: returns an integer less than zero if the string represented by hLeft_string is lexographically smaller than hRight_string. If one string is a prefix of the other string than the shorter string is considered to be the smaller one. (app is less than apple). Returns zero if the strings are the same and returns a number greater than zero if the string represented by hLeft_string is bigger than hRight_string. 
...
//   Function will return SUCCESS if a non-empty string is read successfully and failure otherwise.  Remember that the incoming string may aleady contain some data and this function should replace the data but not necessarily resize the array unless needed.
...
//Precondition: hMy_string is the handle to a valid My_string object. Postcondition: Removes the last character of a string in constant time. Guaranteed to not cause a resize operation of the internal data.  Returns SUCCESS on success and FAILURE if the string is empty.
...
//Precondition:current_word_family, new_key and word are all handles to valid MY_STRING opaque objects. Guess is an alphabetical character that can be either  upper or lower case.
//Postcondition:  Returns SUCCESS after replacing the string in new_key with the key value formed by considering the current word family, the word and the guess.Returns failure in the case of a resizing problem with the new_key string.
